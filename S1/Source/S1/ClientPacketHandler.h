...
public:
	static void Init()
	{
		...
		GPacketHandler[PKT_S_SAVE] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<Protocol::S_SAVE>(Handle_S_SAVE, session, buffer, len); };
	...
	static SendBufferRef MakeSendBuffer(Protocol::C_SAVE& pkt) { return MakeSendBuffer(pkt, PKT_C_SAVE); }
